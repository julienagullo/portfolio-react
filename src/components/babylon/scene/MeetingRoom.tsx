...
import { MEETING_ITEMS, type ItemHover, type Language } from '../../../config/config';
...
export default function MeetingRoom({
  ...
  const layerNames = Object.keys(assets)
    ...
    .sort();
  const itemSprites = Object.keys(assets).filter((name) => name in MEETING_ITEMS);
  ...
  return (
    <>
      ...
      {itemSprites.map((name) => {
        const { x, y, z, width, labelOffsetY, cellWidth, cellHeight, frameCount } = MEETING_ITEMS[name];
        return (
          <SpriteItemMesh
            key={name}
            name={name}
